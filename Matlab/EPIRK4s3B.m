function [u_epirk3, u_epirk4, num_rhs_calls] = EPIRK4s3B(u, dt, RHS_func, c, Gamma, rel_tol)
    %%% ---------------------------------------------------
    %
    % Parameters
    % ----------
    ...
    %%% Internal stage 1; a = u + 2/3 phi_1(1/2 J(u) dt) f(u) dt
    ...
    %%% Internal stage 2; b = u + phi_1(3/4 J(u) dt) f(u) dt
    [b_n_f, rhs_calls_3, ~] = real_Leja_phi(u, 3*dt/4, RHS_func, f_u, c, Gamma, @phi_1, rel_tol);
    b_n = u + (b_n_f * dt);
    ...
    [u_nl_4a, rhs_calls_5, ~] = real_Leja_phi(u, dt, RHS_func, (-38*Nonlin_u + 54*Nonlin_a - 16*Nonlin_b), c, Gamma, @phi_3, rel_tol);
    [u_nl_4b, rhs_calls_6, ~] = real_Leja_phi(u, dt, RHS_func, (180*Nonlin_u - 324*Nonlin_a + 144*Nonlin_b), c, Gamma, @phi_4, rel_tol);

    %%% u_3 = u + phi_1(J(u) dt) f(u) dt + 8 phi_3(J(u) dt) (F(a) - F(u)) dt
    u_epirk3 = u + (u_flux * dt) + (u_nl_3 * dt);

    %%% u_4 = u + phi_1(J(u) dt) f(u) dt + phi_3(J(u) dt) (-38F(u) + 54F(a) - 16F(b)) dt
    %%%                                  + phi_4(J(u) dt) (180F(u) - 324F(a) + 144F(b)) dt
