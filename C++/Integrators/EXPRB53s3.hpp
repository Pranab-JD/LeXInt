...
embedded_solutions<state> EXPRB53s3(rhs& RHS,                 //? RHS function
                                    ...
                                    )
{
    ...
    vector<state> u_flux = real_Leja_phi(RHS, u, rhs_u, {0.5, 0.9, 1.0}, N, phi_1, Leja_X, c, Gamma, tol, dt);
    ...
    vector<state> b_nl = real_Leja_phi(RHS, u, R_a, {0.5, 0.9}, N, phi_3, Leja_X, c, Gamma, tol, dt);

    //? Internal stage 2; b = u + 9/10 phi_1(9/10 J(u) dt) f(u) dt + (27/25 phi_3(1/2 J(u) dt) + 729/125 phi_3(9/10 J(u) dt)) R(a) dt
    state b = axpby(1.0, u, 0.9, u_flux[1], 27.0/25.0, b_nl[0], 729.0/125.0, b_nl[1], N);
    ...
    vector<state> u_nl_4_3 = real_Leja_phi(RHS, u, R_3a, {1.0}, N, phi_3, Leja_X, c, Gamma, tol, dt);

    //? phi_3(J(u) dt) (18R(a) - 250/81R(b)) dt
    vector<state> u_nl_5_3 = real_Leja_phi(RHS, u, R_3b, {1.0}, N, phi_3, Leja_X, c, Gamma, tol, dt);

    //? phi_4(J(u) dt) (-60R(a) + (500/27)R(b)) dt
    vector<state> u_nl_5_4 = real_Leja_phi(RHS, u, R_4,  {1.0}, N, phi_4, Leja_X, c, Gamma, tol, dt);

    //? 3rd order solution; u_3 = u + phi_1(J(u) dt) f(u) dt + phi_3(J(u) dt) (2R(a) + (150/81)R(b)) dt
    state u_exprb3 = axpby(1.0, u, 1.0, u_flux[2], 1.0, u_nl_4_3[0], N);

    //? 5th order solution; u_5 = u + phi_1(J(u) dt) f(u) dt + phi_3(J(u) dt) (18R(a) - (250/81)R(b)) dt + phi_4(J(u) dt) (-60R(a) + (500/27)R(b)) dt
    state u_exprb5 = axpby(1.0, u, 1.0, u_flux[2], 1.0, u_nl_5_3[0], 1.0, u_nl_5_4[0], N);
