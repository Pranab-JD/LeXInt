...
embedded_solutions<state> EXPRB54s4(rhs& RHS, state& u, int N, vector<double>& Leja_X, double c, double Gamma, double tol, double dt, int Real_Imag)
{
    //* -------------------------------------------------------------------------
    ...
    state u_flux_1 = real_Leja_phi(RHS, u, rhs_u, {0.25}, N, phi_1, Leja_X, c, Gamma, tol, dt);
    state u_flux_2 = real_Leja_phi(RHS, u, rhs_u, {0.50}, N, phi_1, Leja_X, c, Gamma, tol, dt);
    state u_flux_3 = real_Leja_phi(RHS, u, rhs_u, {0.90}, N, phi_1, Leja_X, c, Gamma, tol, dt);
    state u_flux_4 = real_Leja_phi(RHS, u, rhs_u, {1.00}, N, phi_1, Leja_X, c, Gamma, tol, dt);

    //? Internal stage 1; a = u + 1/4 phi_1(1/4 J(u) dt) f(u) dt
    state a_n = axpby(1.0, u, 0.25, u_flux_1, N);
    ...
    //? Vertical interpolation of R(a) at 1/2
    state b_nl = real_Leja_phi(RHS, u, R_a, {0.5}, N, phi_3, Leja_X, c, Gamma, tol, dt);

    //? Internal stage 2; b = u + (1/2) phi_1(1/2 J(u) dt) f(u) dt + 4 phi_3(1/2 J(u) dt) R(a) dt
    state b_n = axpby(1.0, u, 0.5, u_flux_2, 4.0, b_nl, N);

    //? Nonlinear remainder at b
    state NL_b = Nonlinear_remainder(RHS, u, b_n, N);
    state R_b = axpby(dt, NL_b, -dt, NL_u, N);

    //? Vertical interpolation of R(b) at 9/10
    state c_nl = real_Leja_phi(RHS, u, R_b, {0.9}, N, phi_3, Leja_X, c, Gamma, tol, dt);

    //? Internal stage 2; b = u + (9/10) phi_1(9/10 J(u) dt) f(u) dt + (729/125) phi_3(9/10 J(u) dt) R(b) dt
    state c_n = axpby(1.0, u, 0.9, u_flux_3, 729.0/125.0, c_nl, N);
    ...
    state u_nl_4_3 = real_Leja_phi(RHS, u, R_4a, {1.0}, N, phi_3, Leja_X, c, Gamma, tol, dt);

    //* phi_4(J(u) dt) (-60R(a) - (285/8)R(b) + (125/8)R(c)) dt
    state u_nl_4_4 = real_Leja_phi(RHS, u, R_4b, {1.0}, N, phi_4, Leja_X, c, Gamma, tol, dt);

    //* phi_3(J(u) dt) (18R(a) - (250/81)R(b)) dt
    state u_nl_5_3 = real_Leja_phi(RHS, u, R_5a, {1.0}, N, phi_3, Leja_X, c, Gamma, tol, dt);

    //* phi_3(J(u) dt) (-60R(a) + (500/27)R(b)) dt
    state u_nl_5_4 = real_Leja_phi(RHS, u, R_5b, {1.0}, N, phi_4, Leja_X, c, Gamma, tol, dt);

    //? 4th order solution; u_4 = u + phi_1(J(u) dt) f(u) dt + phi_3(J(u) dt) (64R(a) - 8R(b)) dt + phi_4(J(u) dt) (-60R(a) - (285/8)R(b) + (125/8)R(c)) dt
    state u_exprb4 = axpby(1.0, u, 1.0, u_flux_4, 1.0, u_nl_4_3, 1.0, u_nl_4_4, N);

    //? 5th order solution; u_5 = u + phi_1(J(u) dt) f(u) dt + phi_3(J(u) dt) (18R(a) - (250/81)R(b)) dt + phi_4(J(u) dt) (-60R(a) + (500/27)R(b)) dt
    state u_exprb5 = axpby(1.0, u, 1.0, u_flux_4, 1.0, u_nl_5_3, 1.0, u_nl_5_4, N);
