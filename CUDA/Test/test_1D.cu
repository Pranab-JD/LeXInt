...
int main()
{
    ...
    double t_final = 0.005;                           // Final simulation time
    ...
    if (problem == "Diff_Adv_1D")
    {
        ...
        for (int ii = 0; ii < N; ii++)
        {
            u[ii] = 1 + exp(-(X[ii] - 0.2)*(X[ii] - 0.2)/0.01);
        }
    }
    else if (problem == "Burgers_1D")
    {
        ...
        for (int ii = 0; ii < N; ii++)
        {
            u[ii] = 2 + 0.01*sin(2*M_PI*X[ii]) + 0.01*sin(8*M_PI*X[ii] + 0.3);
        }
    }
    else
    {
    ...
    // int sys_value = system(("mkdir -p ../../LeXInt_Test/DA_1_GPU/"));
    // string directory = "../../LeXInt_Test/DA_1_GPU/";
