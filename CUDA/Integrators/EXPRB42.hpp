...
embedded_solutions<state> EXPRB42(rhs& RHS,                 //? RHS function
                                  ...
                                  )
{
    ...
    vector<state> u_flux = real_Leja_phi(RHS, u, rhs_u, {3./4., 1.0}, N, phi_1, Leja_X, c, Gamma, tol, dt);

    //? Internal stage 1; a = u + 3/4 phi_1(3/4 J(u) dt) f(u) dt
    state a = axpby(1.0, u, 3./4., u_flux[0], N);
    ...
    state u_exprb2 = axpby(1.0, u, 1.0, u_flux[1], N);

    //? 4th order solution; u_4 = u_2 + 32/9 phi_3(J(u) dt) R(a) dt
    state u_exprb4 = axpby(1.0, u_exprb2, 32./9., u_nl_3[0], N);
