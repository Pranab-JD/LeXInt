...
def imag_Leja_phi(u, dt, RHS_function, interp_vector, integrator_coeffs, c, Gamma, Leja_X, phi_function, tol):
    """
    Computes the polynomial interpolation of 'phi_function' applied to 'interp_vector' at imaginary Leja points.
    ...
    polynomial_array = np.zeros((len(interp_vector), num_interpolations), dtype = "complex")    # Polynomial array
    ...
    for ij in range(0, num_interpolations):
        ...
        phi_function_array[:, ij] = phi_function(integrator_coeffs[ij] * (c + Gamma*Leja_X) * 1j)

        ### Compute polynomial coefficients
        poly_coeffs[:, ij] = Divided_Difference(Leja_X, phi_function_array[:, ij]) 

        ### p_0 term
        polynomial_array[:, ij] = interp_vector * poly_coeffs[0, ij] + 0*1j
    ...
    for ii in range(1, max_Leja_pts):
        ...
        y = (-1j * Jacobian_function * dt/Gamma) + (y * (-c/Gamma - Leja_X[ii - 1]))
        ...
        for ij in range(0, num_interpolations):
            ...
            if poly_error > 1e17:
                ...
                return np.real(polynomial_array), ii+1, convergence
